...
<body>
	<div class="container">
		...
		<s:if test="%{#request.tasks != null and #request.tasks.size() > 0}">
			<h2><s:text name="pending" /> <s:text name="tasks" /></h2>
				...
				<s:iterator value="#request.tasks" var="task">
					<s:if test="#task.pending==0"><s:text name="task" /> <s:text name="approved" />
						...
						<a href="#" class="up-link"
							data-url="<s:property value='rejectUrl' />"> <s:text name="approved" /> </a>
					</s:if>
					<s:if test="#task.pending==2">Task rejected
						...
						<div class="task-info">
							...
							<div>
								<strong><s:text name="status" />:</strong> <select class="status-dropdown"
									...
									name="statusId" <s:if test="#task.pending==1">disabled</s:if>>
